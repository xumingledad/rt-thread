...
void GetWeather()
{
    fopen("/flash/city.txt", "wt+");
    FILE            *fp = NULL;
    char            line[2048] = {0};
    cJSON *root = RT_NULL, *item = RT_NULL;
    webclient_get_file("http://tianqiapi.com/api?version=v6&appid=79931957&appsecret=dBq4JGRZ","/flash/city.txt");
    if(NULL != (fp = fopen("/flash/city.txt", "r")))
    {
		...
		FILE*WEA=fopen("/flash/weather.txt", "wt+");
    ...
    FILE            *fp1 = NULL;
    char            line1[1024] = {0};
    webclient_get_file(weather_url,"/flash/weather.txt");
    if(NULL != (fp1= fopen("/flash/weather.txt", "r")))
    {
